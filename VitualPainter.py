...
from src.main import recognizeImg



# Specifying the text and load OCR model
text = ""
model = getModel()
...
while True:
    ...
    if callRec:
        paintedText =  recognizeImg(model,imgGray)
        print(paintedText)
        if paintedText :
            text = paintedText          
    cv2.putText(img, f'OCR: {text}', (200, 200), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
